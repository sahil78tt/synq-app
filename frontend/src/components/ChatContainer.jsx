...
const Message = memo(function Message({ message, isMine, senderAvatar, senderName }) {
  return (
    <div className={`flex gap-2.5 items-end message-enter ${isMine ? "flex-row-reverse" : "flex-row"}`}>
      {!isMine && (
        <img
...
export default function ChatContainer() {
  const { messages, isLoadingMessages, selectedChat } = useChatStore();
